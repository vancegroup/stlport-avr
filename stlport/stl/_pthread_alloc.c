...
::_S_chunk_alloc(size_t __p_size, size_t &__nobjs) {
  {
    ...
    if (__bytes_left >= __total_bytes) {
      ...
      if (__bytes_left > 0) {
        _Pthread_alloc_per_thread_state<_Max_size>* __a = (_Pthread_alloc_per_thread_state<_Max_size>*)
...
::_M_refill(size_t __n) {
  ...
  __my_free_list = __free_list + _Pthread_alloc<_Max_size>::_S_freelist_index(__n);
...
::reallocate(void *__p, size_t __old_sz, size_t __new_sz) {
  ...
  if (__old_sz > _Max_size && __new_sz > _Max_size) {
