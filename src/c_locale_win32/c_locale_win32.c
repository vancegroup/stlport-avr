...
void* _Locale_time_create(const char * name)
{
    ...
    for(dayofweek=LOCALE_SDAYNAME1; dayofweek<=LOCALE_SDAYNAME7; dayofweek++)
    {
    	...
    	ltime->dayofweek[dayofweek-LOCALE_SDAYNAME1]=(char*)malloc(size);
        if(!ltime->dayofweek[dayofweek-LOCALE_SDAYNAME1]) { _Locale_time_destroy(ltime); return NULL; }
	    GetLocaleInfoA(ltime->lcid, dayofweek, ltime->dayofweek[dayofweek-LOCALE_SDAYNAME1], size);
    	__ConvertFromACP(ltime->dayofweek[dayofweek-LOCALE_SDAYNAME1], size, ltime->cp);
    }

    for(dayofweek=LOCALE_SABBREVDAYNAME1; dayofweek<=LOCALE_SABBREVDAYNAME7; dayofweek++)
    {
	    size=GetLocaleInfoA(ltime->lcid, dayofweek, NULL, 0);
    	ltime->abbrev_dayofweek[dayofweek-LOCALE_SABBREVDAYNAME1]=(char*)malloc(size);
        if(!ltime->abbrev_dayofweek[dayofweek-LOCALE_SABBREVDAYNAME1]) { _Locale_time_destroy(ltime); return NULL; }
	    GetLocaleInfoA(ltime->lcid, dayofweek, ltime->abbrev_dayofweek[dayofweek-LOCALE_SABBREVDAYNAME1], size);
    	__ConvertFromACP(ltime->abbrev_dayofweek[dayofweek-LOCALE_SABBREVDAYNAME1], size, ltime->cp);
    }
